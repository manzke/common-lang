...
public final class ObfuscatedString {
    ...
    public static ObfuscatedString obfuscate( final String s ) {
        if ( -1 != s.indexOf( 0 ) ) {
            throw new IllegalArgumentException( new ObfuscatedString( Arrays.asList( "241005931110FC70",
                                                                                     "DCD925A88EAD9F37",
                                                                                     "19ADA1C861E2A85D",
                                                                                     "9A5948E700FCAD8A",
                                                                                     "2E11C83A72441DE2" ) ).toString() ); // =>
    ...
    /**
     * Decodes a long value from eight bytes in little endian order, beginning at index {@code off}. This is the inverse
     * of {@link #toBytes(long, byte[], int)}. If less than eight bytes are remaining in the array, only these low order
     * bytes are processed and the complementary high order bytes of the returned value are set to zero.
     * 
    ...
    /**
     * Encodes a long value to eight bytes in little endian order, beginning at index {@code off}. This is the inverse
     * of {@link #toLong(byte[], int)}. If less than eight bytes are remaining in the array, only these low order bytes
     * of the long value are processed and the complementary high order bytes are ignored.
     * 
     ...
     * @throws NullPointerException If {@code obfuscated} is {@code null}.
     * @throws ArrayIndexOutOfBoundsException If the provided array does not contain at least one element.
